...
function formatGG2AT(obj, cardname) {
  ...
  if (!match || match.cardname.toLowerCase() !== cardname.toLowerCase()) {
    return [];
  }
...
export default async function scrapeGoodGames(cardURI) {
  ...
  try {
    const { data } = await axios.get(
      `${URL}/search?q=${cardURI}&f_Availability=Exclude Out Of Stock`,
      {
        headers: {
          "User-Agent":
    ...
    const raw = $(
      'script:contains("Spurit.Preorder2.snippet.products")',
    ).html();
    const regex =
      /Spurit\.Preorder2\.snippet\.products\['([^']+)'\]\s*=\s*(\{.*?\});/gs;
